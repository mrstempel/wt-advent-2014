...
{
	overlayPrefix = 'map';
	$('#zoom-control').css('display', 'inline-block');
	$('#home-btn').css('display', 'inline-block');
	if (map == null)
	{
		showLoadingMsg();
		...
		{
			if (questionMarker != null && questionMarker.getMap() != null && questionFeedbackBubble.getMap() == null && (targetMarker == null || targetMarker.getMap() == null))
			{
				debug("recenter question marker");
				//questionMarker.setPosition(map.getCenter());
			}
		});
	...
	$.ajax({
		...
		success: function (data)
		{
			if (data)
			{
				...
				if (loadPrev)
				{
					//setCurrentQuestion(data[data.length - 1]);
				...
				if (isQuestionAnswered(currentQuestion))
				{
					...
					google.maps.event.addListener(targetMarker, 'click', function () { showMoreInfo(currentQuestion.Question.Day, 0, false); });
				...
				else
				{
					...
					var myIcon = "/images/eumel/dragger-rot-" + mySnowieVersion + ".png";
					questionMarker = new google.maps.Marker({
						position: questionPosition,
						title: currentQuestion.TranslatedTitle,
						icon: new google.maps.MarkerImage(myIcon, null, null, null, new google.maps.Size(55, 55)),
						map: map,
						shadow: markerShadow,
						draggable: true
					});
					...
					setZoom(currentQuestion.Question.ZoomLevel);
					map_recenter(questionMarker.position, 0, -120);

					$('#question > .container > h1').html(currentQuestion.TranslatedTitle);
					$('#question').fadeIn();
					$('#question-how-to1').fadeIn();
...
function addAnsweredMarker(q)
{
	...
	{
		setMediaPixel('568030'); _gaq.push(['_trackEvent', 'mehr-lesen-tag-' + q.Question.Day, 'clicked']); showMoreInfo(q.Question.Day, 0, false);
	});
...
function showCorrectAnswer(tryCount, bonusPoints)
{
	...
	{
		map_recenter(targetMarker.position, 250, 0);
	}
	//zoomOut();
	//map_recenter(targetMarker.position, 250, 0);
	...
	window.setTimeout(function () { hideBonus('answer-points', false); }, 1300);
}

function showAnswerMoreInfo(points)
{
	debug("showAnswerMoreInfo called");
	var content = $('#answer-confirm').html().replace("{more-info-link}", "_gaq.push(['_trackEvent', 'mehr-lesen-tag-" + currentQuestion.Question.Day + "', 'clicked']);showMoreInfo(" + currentQuestion.Question.Day + ", " + points + ", true);");
	content = content.replace("{more-info-bonus-message}", moreInfoBonusMsg);
	answerFeedbackBubble.setContent(content);
	answerNewLat = targetMarker.getPosition().lat() - (0.000055 * Math.pow(2, (21 - map.getZoom())));
